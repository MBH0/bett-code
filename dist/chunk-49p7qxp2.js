...
} from "./chunk-cdm37krk.js";
import {
  CCRClient,
  CCRInitError,
  SSETransport
} from "./chunk-bp6cmz9z.js";
import {
  StructuredIO,
  ...
  sdkCompatToolName
} from "./chunk-4szkve5z.js";
import {
  detectAndUninstallDelistedPlugins
} from "./chunk-x2jr5k20.js";
import {
...
async function runHeadless(inputPrompt, getAppState, setAppState, commands, tools, sdkMcpConfigs, agents, options) {
  ...
  try {
    ...
    fs.appendFileSync("/tmp/bett-debug.log", `[print.ts] runHeadless entered
`);
  } catch {}
  try {
    const fs = await import("fs");
    fs.appendFileSync("/tmp/bett-debug.log", `[print.ts] before grove check
`);
  } catch {}
  ...
  try {
    const fs = await import("fs");
    fs.appendFileSync("/tmp/bett-debug.log", `[print.ts] after grove check
`);
  } catch {}
  initializeGrowthBook();
...
`);
  ...
  try {
    const fs = await import("fs");
    fs.appendFileSync("/tmp/bett-debug.log", `[print.ts] before structuredIO
`);
  } catch {}
...
`);
  ...
  try {
    const fs = await import("fs");
    fs.appendFileSync("/tmp/bett-debug.log", `[print.ts] before loadInitialMessages
`);
  } catch {}
  const appState = getAppState();
  const {
  ...
  try {
    const fs = await import("fs");
    fs.appendFileSync("/tmp/bett-debug.log", `[print.ts] after loadInitialMessages, msgs=${initialMessages?.length}
`);
  } catch {}
  ...
  if (initialMessages.length === 0 && process.exitCode !== undefined) {
    try {
      const fs = await import("fs");
      fs.appendFileSync("/tmp/bett-debug.log", `[print.ts] EARLY RETURN: msgs=0, exitCode=${process.exitCode}
`);
    } catch {}
    return;
  }
...
function runHeadlessStreaming(structuredIO, mcpClients, commands, tools, initialMessages, canUseTool, sdkMcpConfigs, getAppState, setAppState, agents, options, turnInterruptionState) {
  ...
  try {
    __require("fs").appendFileSync("/tmp/bett-debug.log", `[print.ts] before modelOptions
`);
  } catch {}
  ...
  const run = async () => {
    ...
    if (pluginInstallPromise) {
      ...
      const { setupPluginHookHotReload } = await import("./chunk-8z885w47.js");
      ...
      const drainCommandQueue = async () => {
        while (command = dequeue(isMainThread)) {
          ...
          await runWithWorkload(cmd.workload ?? options.workload, async () => {
            try {
              const fs = await import("fs");
              fs.appendFileSync("/tmp/bett-debug.log", `[print.ts] calling ask()
`);
            } catch {}
...
`);
  ...
  (async () => {
    ...
    try {
      __require("fs").appendFileSync("/tmp/bett-debug.log", `[print.ts] starting input loop
`);
    } catch {}
    for await (const message of structuredIO.structuredInput) {
      try {
        __require("fs").appendFileSync("/tmp/bett-debug.log", `[print.ts] got message type=${message.type}
`);
      } catch {}
...
`);
          ...
          if (message.request.enabled) {
            if (bridgeHandle) {
              ...
              try {
                const { initReplBridge } = await import("./chunk-52wtf7yr.js");
...
async function loadInitialMessages(setAppState, options) {
  try {
    const fs = await import("fs");
    fs.appendFileSync("/tmp/bett-debug.log", `[loadInitialMessages] entered, SIMPLE=${process.env.CLAUDE_CODE_SIMPLE}, continue=${options.continue}, resume=${options.resume}
`);
  } catch {}
  if (process.env.CLAUDE_CODE_SIMPLE === "1" && !options.continue && !options.resume) {
    try {
      const fs = await import("fs");
      fs.appendFileSync("/tmp/bett-debug.log", `[loadInitialMessages] early return (SIMPLE mode)
`);
    } catch {}
    return { messages: [] };
  }
  ...
  if (options.teleport) {
    try {
      ...
      const {
        ...
        validateGitState
      } = await import("./chunk-br16a41c.js");
